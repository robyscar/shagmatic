/*    NOTES
 *     Optional code is commented in place so it can be tried easily
 *     Lookup tables values can be produced an copied with the XLS Values and divider must be determined experimentally
 *     as stepper paramteres, pulley or wheel size and analog reference voltage all contribute to the scaling
 *     Very strange behavior may result if digital voltage does not match that of the controller. Be sure to use 3.3 or 5 volts 
 *     as needed. If the encoders require 5 volts and the micro is not 5 volt tolerant, a signal converter will be needed
 *     to protect the controller inputs.
 *     limit switch avoidance may be done with an offset value incremented when limit switches are hit, this is currently 
 *     commented out. It may also be possible to track absolute position but have not figured this out yet.
 *     This version has been tested on my cable drive system but can be adapted with different table and scaling 
 *     values to other machines. I am using a teensy 3.2 with 5 volt digital inputs and an external analog voltage reference
 *     of 3.3 volts taken from the 3.3 volt regulated output of the teensy. a more precise reference from a shunt regulator might 
 *     be more stable. A 1 up cap is across each analog input and ground to filter the voltage and this seems adequate. software
 *     debouncing has also been tried but does not seem necessary. Looping the cable to the joystick through a ferrite is necessary 
 *     to reduce noise which can cause jitter. Two loops seems best. Ferrites are also necessary on the limit switch lines especially 
 *     if they are long and or go near stepper wires.
*/  
...
int offset = 0;
int min_stroke = 10; // used to determine at what setting motion will begin and estop recovery occurs
...
// pin 0 will be used for audio decoder direction output, not sure what switch 8 does?
  ...
  if (digitalReadFast(11)== HIGH){  // only do seek home if switch set for option
      if (digitalReadFast(9)== LOW) {
           for (int i=0; i < 100; i++){ // be sure to be forward of rear switch
            cwpulse_quad();
            delay (1);
           }
      }
      on_switch = false;
      while (on_switch == false){ // using this method with a boolean just to see if it works can also use while (digitalReadFast(9)== HIGH)
             ccwpulse_quad();  //      ccwpulse();  // ccw is retract on my machine
             if (digitalReadFast(9)== LOW) on_switch = true;
             delay (1);        
       }   
    }
    for (int i=0; i < 10; i++){ // then move forward again to be off switch
      cwpulse_quad();
      delay (1);
    }    
    current_position = 10;
...
//   if (digitalReadFast(9)== LOW)  current_position = min_position;            // probably not needed with new limit switch control but left in case it is needed
   while (digitalReadFast(10)== LOW){
        ...
        delay (1);
        origin_offset +=  joystick_encoder_multiplier;        
   ...
   while (digitalReadFast(9)== LOW){
       if (quad_out){       // not really necessary because this is so slow but WTF?
        ...
        delay (1);
        end_offset +=  joystick_encoder_multiplier; 
        read_joystick_encoder();      // probably not needed
        read_pots();
   }
   if (run){    // if three knob is in use can try with and without end and origin offset can also try absolute posistion tracking
//        if (forward == true) {stepper.moveTo (offset_plus_stroke - end_offset);}  // only need to move to new offset position if there is no stroke
//            else{stepper.moveTo(offset + origin_offset);}       
...
//         if (stroke < vibe_limit) {motor_speed = constrain (motor_speed,0,max_speed_vibe); } 
         ...
         int offset_index = (analogRead(20)) / 10;  // table has 103 entries     
...
void cwpulse(void){  // cw is forward on my machine
...
void ccwpulse(void){  // ccw is retract on my machine
...
void cwpulse_quad(void){  // combined pulse and quad out  cw is forward on my machine
    digitalWriteFast(4, LOW); // pin 4 low for CW 
...
void ccwpulse_quad(void){  // combined pulse and quad out  // ccw is retract on my machine
    digitalWriteFast(4, HIGH); // pin 4 high for CCW 
    delayMicroseconds(1);
    for (int i=0; i < joystick_encoder_multiplier; i++){
...
//         static int last_stroke;
//         static int last_offset;
//         int diff_time = time - last_time;
//         int diff_speed;
//         int diff_stroke;
//         int diff_offset;
//         int cpm_thresh = 1;    // just a guess need to figure this out
//         last_time = time;
//         if (loop_count > 10){  // if just started or estopped do not check because values can be not initialized and or read
//            int all_diff = diff_speed + diff_stroke + diff_offset;
//            int change_perms = all_diff / diff_time;
////            if (change_perms  > cpm_thresh){
////                estop();
////            }
//         }         
...
//         diff_speed = (motor_speed - last_speed);
//         diff_stroke = abs(stroke - last_stroke);
//         diff_offset = abs(offset - last_offset);
